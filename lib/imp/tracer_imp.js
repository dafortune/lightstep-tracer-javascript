...
var Tracer = function (_opentracing$Tracer) {
    ...
    function Tracer(opts) {
        ...
        _this._spanRecords = new Array(500);
        _this._spanRecordsLength = 0;
    ...
    _createClass(Tracer, [{
        ...
        value: function _clearBuffers() {
            this._spanRecords = new Array(500);
            this._spanRecordsLength = 0;
            this._internalLogs = [];
        ...
        value: function _internalAddSpanRecord(record) {
            ...
            if (this._spanRecordsLength >= this._options.max_span_records) {
                var index = Math.floor(this._spanRecordsLength * Math.random());
                this._spanRecords[index] = record;
                this._counters['spans.dropped']++;
            } else {
                this._spanRecords.push(record);
                this._spanRecordsLength += 1;
            }
        }
    }, {
        key: '_restoreRecords',
                ...
                _this10._flushReport(false, true, function (err) {
                    if (err) {
                        _this10._warn('Final report before exit failed', {
                            error: err,
                            unflushed_spans: _this10._spanRecordsLength,
        ...
        value: function _flushReport(manual, detached, done) {
            ...
            var spanRecordsLength = this._spanRecordsLength;
            ...
            if (this._useClockState && !manual && !clockReady && !detached) {
                ...
                // variables
                this._clearBuffers();
                this._debug('Flushing report (' + spanRecordsLength + ' spans)');
            }
